 ...
 * Uses only Node.js built-ins — no Electron dependencies.
 */
...
import type { ScreenControlParams, ScreenControlResult } from "./types.js";
...
/**
 * Build env object for spawning powershell.exe.
 * On WSL, custom env vars must be registered in WSLENV to pass through.
 */
function buildEnv(extraEnv: Record<string, string>): NodeJS.ProcessEnv {
  ...
  if (IS_WSL) {
    ...
    if (extraKeys.length > 0) {
      const existing = merged.WSLENV ? merged.WSLENV + ":" : "";
      merged.WSLENV = existing + extraKeys.join(":");
    }
  }
  return merged;
}
...
async function runPowerShell(
  ...
  const { stdout, stderr } = await execFileAsync(
    ...
    { timeout, maxBuffer, windowsHide: true, env: buildEnv(env) }
  );
...
const SW_MAP: Record<string, number> = {
  minimize: 6,
  maximize: 3,
  restore: 9
};
...
async function handleWindowAccessibility(handle: number): Promise<{
  tree: string;
  left: number;
  top: number;
  w: number;
  ...
  try {
    const { stdout } = await runPowerShell(script("window-accessibility.ps1"), {
      timeout: 10000,
      maxBuffer: 1024 * 1024,
