...
export const awkProgram: fc.Arbitrary<string> = fc.oneof(
  ...
  {
    weight: 2,
    arbitrary: fc.tuple(awkRule, awkAction).map(([r, e]) => `${r} END { ${e} }`)
  },
  // Multiple rules
  {
    weight: 2,
...
const awkPollutionExpr: fc.Arbitrary<string> = fc.oneof(
  ...
  {
    weight: 5,
    arbitrary: fc.tuple(awkDangerousVar, awkExpr).map(([v, e]) => `${v} = ${e}`)
  },
...
const jqPollutionExpr: fc.Arbitrary<string> = fc.oneof(
  ...
  {
    weight: 3,
    arbitrary: fc.tuple(jqKey, jqPollutionKey).map(([k, v]) => `.${k} = "${v}"`)
  },
