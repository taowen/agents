...
export class LiteTerminal {
  ...
  private render(forceFullRender = false): void {
    ...
    if (
      ...
      this.lastCursorLine < this.lines.length
    ) {
  ...
  private renderLineContent(lineIndex: number, lineEl: HTMLElement): void {
    ...
    if (!isCursorLine) {
      ...
      for (const segment of line) {
        if (segment.text) {
          lineEl.appendChild(
            this.createStyledSpan(segment.text, segment.style)
          );
        }
      }
    ...
    for (const segment of line) {
      ...
      if (
        !cursorInserted &&
        this.currentCol >= segStart &&
        this.currentCol < segEnd
      ) {
        ...
        if (beforeCursor) {
          lineEl.appendChild(
            this.createStyledSpan(beforeCursor, segment.style)
          );
        }
  ...
  private createTextWithLinks(
    ...
    while ((match = LiteTerminal.URL_REGEX.exec(text)) !== null) {
      ...
      if (match.index > lastIndex) {
        ...
        fragment.appendChild(
          this.createStyledElement(beforeText, classes, inlineStyle)
        );
      }
    ...
    if (lastIndex < text.length) {
      ...
      fragment.appendChild(
        this.createStyledElement(afterText, classes, inlineStyle)
      );
    }
  ...
  private calculateCols(): void {
    ...
    const availableWidth =
      (this.container.clientWidth || window.innerWidth) - containerPadding;
