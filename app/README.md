...
This directory contains the full application — a Cloudflare Worker server paired with a React Native Android agent.
...
| `rn/` | React Native Android agent that connects to ai-chat via device auth + WebSocket |

## How they work together

1. The RN agent authenticates with the server using a device auth flow (similar to OAuth2 Device Authorization)
