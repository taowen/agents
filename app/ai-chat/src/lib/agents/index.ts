 ...
 */
function callable(metadata: CallableMetadata = {}) {
...
> extends Server<Env, Props> {
  ...
  async addMcpServer(
    serverName: string,
    urlOrBinding: string | DurableObjectNamespace<any>,
    ...
    if (typeof urlOrBinding !== "string") {
      ...
      const { id } = await this.mcp.connect(
        `${RPC_DO_PREFIX}${normalizedName}`,
        {
          ...
          transport: {
            type: "rpc" as TransportType,
            namespace: urlOrBinding as unknown as DurableObjectNamespace,
            name: normalizedName,
            props: rpcOpts?.props
          }
        }
      ...
      const bindingName = this._findBindingNameForNamespace(
        urlOrBinding as unknown as DurableObjectNamespace
      );
      if (bindingName) {
