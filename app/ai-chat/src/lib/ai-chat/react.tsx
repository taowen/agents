...
>(
  ...
  clearHistory: () => void;
  /**
   * Update local message state without server sync.
   * Used for prepending lazy-loaded older messages.
   */
  setLocalMessages: ReturnType<typeof useChat<ChatMessage>>["setMessages"];
  /**
        ...
        case MessageType.CF_AGENT_CHAT_MESSAGES:
          setMessages((prevMessages: ChatMessage[]) => {
            if (prevMessages.length === 0) {
              // First load — just accept server state
              return data.messages;
            }
            // Build a map of incoming messages by ID for fast lookup
            const incomingMap = new Map<string, ChatMessage>();
            for (const msg of data.messages) {
              incomingMap.set(msg.id, msg);
            }
            // Start with local messages, updating any that server has newer versions of
            const merged: ChatMessage[] = [];
            const seenIds = new Set<string>();
            for (const localMsg of prevMessages) {
              const serverVersion = incomingMap.get(localMsg.id);
              merged.push(serverVersion ?? localMsg);
              seenIds.add(localMsg.id);
            }
            // Append any new messages from server that aren't in local state
            for (const msg of data.messages) {
