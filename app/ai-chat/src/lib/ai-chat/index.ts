...
> extends Agent<Env, State> {
  ...
  /**
   * Number of most-recent messages to send to newly connected clients.
   * When set, `onConnect` and `/get-messages` will return only the last N messages
   * instead of the full history. Clients can fetch older messages via pagination.
   *
   * Set to `undefined` (default) to send all messages — backwards-compatible.
   */
  initialMessageCount: number | undefined = undefined;
  ...
  constructor(ctx: AgentContext, env: Env) {
    ...
    this.onConnect = async (connection: Connection, ctx: ConnectionContext) => {
      // Push empty messages on connect. Initial message loading is handled
      // by the HTTP /get-messages endpoint; sending the full history here
      // would be redundant and waste bandwidth.
      connection.send(
        JSON.stringify({
    ...
    this.onRequest = async (request: Request) => {
      return this._tryCatchChat(async () => {
        ...
        if (url.pathname.split("/").pop() === "get-messages") {
          ...
          if (beforeId) {
            // Pagination: return `limit` messages before the given ID
            const limit = limitParam ? parseInt(limitParam, 10) : 50;
            const result = this._getMessagesPaginated(beforeId, limit);
            return Response.json(result);
          }

          // Default: return most recent messages (or all if initialMessageCount not set)
          const messages =
            this.initialMessageCount != null
              ? this.messages.slice(-this.initialMessageCount)
              : this.messages;
  ...
  /**
   * Returns up to `limit` messages that come before the message with `beforeId`
   * in the in-memory messages array. Used for lazy-loading older messages.
   *
   * @param beforeId - The ID of the message to paginate before
   * @param limit - Maximum number of messages to return
   * @returns Array of messages preceding the given ID, ordered by creation time
   */
  private _getMessagesPaginated(
    beforeId: string,
    limit: number
  ): ChatMessage[] {
    const idx = this.messages.findIndex((m) => m.id === beforeId);
    if (idx <= 0) return []; // Not found or already at the beginning
    const start = Math.max(0, idx - limit);
    return this.messages.slice(start, idx);
  }
